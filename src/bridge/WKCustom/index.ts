    ...
    */
    ...
    constructor() {
        ...
        this.receiveMessageFromNative(NativeWebBridgeMethod.getShowTextContent,
            ['whiteboard.error.textIsOtherEdited', 'error.courseware.limit.length',
        ...
        UserOptionsUtils.setIsTeacherCallback(() => {
            return new Promise(async (resolve) => {
                const result = await this.receiveMessageFromNative(NativeWebBridgeMethod.getIsTeacher, JSON.stringify({}))
                if(result+'' === 'true') {
                    resolve(true)
        ...
        this.customShowLog("注册WKWindowManager完成，开始初始化WKWindowManagerStore");
        this.wkWindowManagerStore = new WKWindowManagerStore(() => {
            return window.manager as WindowManager
        }, (isInitialized: boolean) => {
            this.customShowLog(`初始化WKWindowManagerStore完成，isInitialized: ${isInitialized}`);
            if (isInitialized) {
    ...
    private onBoxChangeListener = (data: { maxMaxTopBox?: any, maxNomalTopBox?: any }) => {
        ...
        this.customShowLog(`监听焦点缩放比例，data: ${data.maxMaxTopBox} , ${data.maxNomalTopBox}`);
    }
    ...
    private onPageChangeListener = (_appId: string, _scenePath: string) => {
        ...
        this.customShowLog(`监听页面变化，_appId: ${_appId}, _scenePath: ${_scenePath}`);
    }
    ...
    private onMemberStateChangeListener = (memberState: any) => {
        ...
        this.customShowLog(`监听教具状态变化，memberState: ${JSON.stringify(memberState)}`);
    ...
    public sendMessageToNative = async (method: NativeWebBridgeMethod, data?: any) => {
        try {
            ...
            this.customShowLog('sendMessageToNativeError', method, 'error', data, error)
        }
    }
    /**
     * 接收消息来自原生
     * @param data 数据
     */
    public receiveMessageFromNative = async (method: NativeWebBridgeMethod, data: any) => { 
        let result = '';
        try {
            this.customShowLog('receiveMessageFromNative', method, 'start', data)
            result = await (asyncCall(`wuKongOptions.getInfoSync`, JSON.stringify({ method, data:data })) as Promise<string>)
            this.customShowLog('receiveMessageFromNativeResult',method, 'end', data, result)
            return result;
        } catch (error) {
            this.customShowLog('receiveMessageFromNativeError', method, 'error', data, error)
        }
